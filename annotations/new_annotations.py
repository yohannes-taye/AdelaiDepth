...
def create_annotations(csv_file, img_folder, output_folder, rgb_extention):
    ...
    for s in scenes:
        ...
        s_i = os.listdir(os.path.join(path, s, 'rgb'))
        s_d = os.listdir(os.path.join(path, s, 'depth'))
        for d in s_d:
            if d.replace('png', rgb_extention) in s_i:
                depth_path = os.path.join(img_folder, s, 'depth', d)
                rgb_path = os.path.join(img_folder, s, 'rgb', d.replace('png', rgb_extention))
                anno = {}
...
if __name__ == '__main__':
    ...
    parse_args.add_argument('--debug', default=False, action='store_true')
